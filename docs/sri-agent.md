...
3. Crea `~/.sri-agent/venv/` e instala `playwright` + `playwright-stealth` (+ `pip-system-certs` en Windows — ver troubleshooting de SSL más abajo)
...
| Callback falla con `CERTIFICATE_VERIFY_FAILED: certificate has expired` en Windows, pero el navegador/.NET sí conecta bien | El almacén de certificados raíz de Windows tiene una raíz nueva (ej. tras rotación de Let's Encrypt) que Python no ve porque no delega en CryptoAPI como sí hace .NET — afecta a cualquier PC cuyo almacén local no haya cacheado esa raíz todavía, no es un caso aislado | Desde la versión del instalador que incluye `pip-system-certs`, instalaciones nuevas ya vienen con el fix. En instalaciones viejas: `.\venv\Scripts\pip.exe install pip-system-certs` dentro de `~/.sri-agent`, reiniciar el agente |

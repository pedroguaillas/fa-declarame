...
const systemSlugs = ["admin", "employee"];

const deleteDialog = ref(false);
const roleToDelete = ref<TenantRole | null>(null);

function confirmDelete(role: TenantRole) {
...
function handleDelete() {
    ...
    router.delete(route("tenant.roles.destroy", roleToDelete.value.id), {
        onFinish: () => {
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-muted-foreground text-sm mt-1">
                        Gestiona los roles y sus permisos en el tenant.
                    </p>
                </div>
                <Button v-if="can('create', 'roles')" @click="router.visit(route('tenant.roles.create'))">
                    <Plus class="size-4" />
                    Nuevo rol
                </Button>
            </div>
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="role in roles" :key="role.id">
                            ...
                            <TableCell>
                                <code class="text-xs bg-muted px-1.5 py-0.5 rounded">{{ role.slug }}</code>
                            </TableCell>
                            ...
                            <TableCell class="text-center">
                                <Badge variant="outline">{{ role.model_permissions?.length ?? 0 }}</Badge>
                            </TableCell>
                            <TableCell class="text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <Button v-if="can('edit', 'roles')" variant="ghost" size="icon"
                                        :disabled="systemSlugs.includes(role.slug)"
                                        @click="router.visit(route('tenant.roles.edit', role.id))">
                                        <Pencil class="size-4" />
                                    </Button>
                                    <Button v-if="can('delete', 'roles')" variant="ghost" size="icon"

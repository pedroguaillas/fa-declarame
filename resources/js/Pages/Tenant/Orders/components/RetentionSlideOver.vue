...
async function open(order: Order) {
    ...
    const res = await fetch(route("tenant.orders.show", { order: order.id }), {
...
function baseForRetention(type: string): number {
    ...
    if (type === "iva") {
        return (
            Number(order.iva5 ?? 0) +
            Number(order.iva12 ?? 0) +
            Number(order.iva15 ?? 0)
        );
    }
    return Number(order.sub_total ?? 0);
}
...
<template>
    <Teleport to="body">
        ...
        >
            <div v-if="panelOpen" class="fixed inset-0 z-40 bg-black/40" @click="close" />
        </Transition>
        ...
        >
            <div
                v-if="panelOpen && selectedOrder"
                class="bg-background border-border fixed inset-y-0 right-0 z-50 flex w-full max-w-3xl border-l shadow-xl"
            >
                <!-- Left: order info -->
                <div class="border-border flex w-64 shrink-0 flex-col border-r">
                    <div class="border-border border-b px-5 py-4">
                        <p class="text-muted-foreground text-xs font-semibold tracking-widest uppercase">Venta</p>
                    ...
                    <div class="flex-1 space-y-4 overflow-y-auto p-5">
                        ...
                        <div>
                            ...
                            <p class="text-foreground font-mono text-xs break-all">{{ selectedOrder.autorization }}</p>
                        </div>
                        <div class="border-border overflow-hidden rounded-lg border">
                            <table class="min-w-full text-xs">
                                ...
                                <tbody class="divide-border divide-y">
                                    <tr v-if="Number(selectedOrder.no_iva) > 0">
                                        <td class="text-muted-foreground px-3 py-1.5">No IVA</td>
                                        <td class="text-foreground px-3 py-1.5 text-right font-mono">${{ Number(selectedOrder.no_iva).toFixed(2) }}</td>
                                    </tr>
                                    <tr v-if="Number(selectedOrder.base0) > 0">
                                        <td class="text-muted-foreground px-3 py-1.5">Base 0%</td>
                                        <td class="text-foreground px-3 py-1.5 text-right font-mono">${{ Number(selectedOrder.base0).toFixed(2) }}</td>
                                    </tr>
                                    <tr v-if="Number(selectedOrder.base12) > 0">
                                        <td class="text-muted-foreground px-3 py-1.5">Base 12%</td>
                                        <td class="text-foreground px-3 py-1.5 text-right font-mono">${{ Number(selectedOrder.base12).toFixed(2) }}</td>
                                    </tr>
                                    <tr v-if="Number(selectedOrder.iva12) > 0">
                                        <td class="text-muted-foreground px-3 py-1.5 pl-5">IVA 12%</td>
                                        <td class="text-foreground px-3 py-1.5 text-right font-mono">${{ Number(selectedOrder.iva12).toFixed(2) }}</td>
                                    </tr>
                                    <tr v-if="Number(selectedOrder.base15) > 0">
                                        <td class="text-muted-foreground px-3 py-1.5">Base 15%</td>
                                        <td class="text-foreground px-3 py-1.5 text-right font-mono">${{ Number(selectedOrder.base15).toFixed(2) }}</td>
                                    </tr>
                                    <tr v-if="Number(selectedOrder.iva15) > 0">
                                        <td class="text-muted-foreground px-3 py-1.5 pl-5">IVA 15%</td>
                                        <td class="text-foreground px-3 py-1.5 text-right font-mono">${{ Number(selectedOrder.iva15).toFixed(2) }}</td>
                                    </tr>
                                    <tr v-if="Number(selectedOrder.discount) > 0">
                                        <td class="text-muted-foreground px-3 py-1.5">Descuento</td>
                                        <td class="text-destructive px-3 py-1.5 text-right font-mono">-${{ Number(selectedOrder.discount).toFixed(2) }}</td>
                                    </tr>
                                    <tr v-if="Number(selectedOrder.ice) > 0">
                                        <td class="text-muted-foreground px-3 py-1.5">ICE</td>
                                        <td class="text-foreground px-3 py-1.5 text-right font-mono">${{ Number(selectedOrder.ice).toFixed(2) }}</td>
                                    </tr>
                                    <tr class="bg-muted font-semibold">
                                        <td class="text-foreground px-3 py-2">Total</td>
                                        <td class="text-foreground px-3 py-2 text-right font-mono">${{ Number(selectedOrder.total).toFixed(2) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                ...
                <div class="flex min-w-0 flex-1 flex-col">
                    <div class="border-border flex items-start justify-between border-b px-6 py-4">
                        <div>
                            ...
                            <p class="text-muted-foreground mt-0.5 font-mono text-sm">
                                {{ selectedOrder.serie_retention ?? "001-001-000000001" }}
                            </p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div v-if="selectedOrder.serie_retention" class="min-w-0 flex-1 overflow-y-auto p-6">
                        <div class="mb-6 grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <p class="text-muted-foreground mb-1 text-xs font-medium tracking-wider uppercase">Fecha</p>
                                <p class="text-foreground text-sm">{{ selectedOrder.date_retention }}</p>
                            </div>
                            <div class="col-span-2">
                                <p class="text-muted-foreground mb-1 text-xs font-medium tracking-wider uppercase">Autorización</p>
                                <p class="text-foreground font-mono text-sm break-all">{{ selectedOrder.autorization_retention }}</p>
                            </div>
                            <div>
                                <p class="text-muted-foreground mb-1 text-xs font-medium tracking-wider uppercase">Estado</p>
                                <Badge>{{ selectedOrder.state_retention }}</Badge>
                            </div>
                        </div>
                        ...
                        <div class="border-border overflow-auto rounded-lg border">
                            <table class="divide-border min-w-full divide-y text-sm">
                                <thead class="bg-muted">
                                    <tr>
                                        <th class="text-muted-foreground whitespace-nowrap px-4 py-2.5 text-left text-xs font-medium uppercase">Tipo</th>
                                        <th class="text-muted-foreground whitespace-nowrap px-4 py-2.5 text-left text-xs font-medium uppercase">Código</th>
                                        <th class="text-muted-foreground whitespace-nowrap px-4 py-2.5 text-left text-xs font-medium uppercase">Descripción</th>
                                        <th class="text-muted-foreground whitespace-nowrap px-4 py-2.5 text-right text-xs font-medium uppercase">Base</th>
                                        <th class="text-muted-foreground whitespace-nowrap px-4 py-2.5 text-right text-xs font-medium uppercase">%</th>
                                        <th class="text-muted-foreground whitespace-nowrap px-4 py-2.5 text-right text-xs font-medium uppercase">Valor</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-border divide-y">
                                    <tr v-for="item in selectedOrder.retention_items" :key="item.id">
                                        <td class="text-foreground whitespace-nowrap px-4 py-2.5">
                                            {{ typeLabel[item.retention?.type ?? ""] ?? item.retention?.type }}
                                        </td>
                                        <td class="text-foreground whitespace-nowrap px-4 py-2.5 font-mono">{{ item.retention?.code }}</td>
                                        <td class="text-muted-foreground px-4 py-2.5 text-sm">{{ item.retention?.description }}</td>
                                        <td class="text-foreground whitespace-nowrap px-4 py-2.5 text-right font-mono">${{ Number(item.base).toFixed(2) }}</td>
                                        <td class="text-foreground whitespace-nowrap px-4 py-2.5 text-right font-mono">{{ item.percentage }}%</td>
                                        <td class="text-foreground whitespace-nowrap px-4 py-2.5 text-right font-mono font-medium">${{ Number(item.value).toFixed(2) }}</td>
                                    </tr>
                                </tbody>
                        ...
                        <div class="flex-1 space-y-5 overflow-y-auto p-6">
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="col-span-2 flex flex-col gap-1.5">
                                    ...
                                    />
                                    <p v-if="retentionForm.errors.autorization_retention" class="text-destructive text-xs">
                            ...
                            <div>
                                <div class="mb-3 flex items-center justify-between">
                                    <p class="text-muted-foreground text-xs font-medium tracking-wider uppercase">Detalle de retenciones</p>
                                ...
                                <div class="space-y-2">
                                    ...
                                    >
                                        <div class="relative mb-2">
                                            ...
                                            <Teleport to="body">
                                                <div
                                                    v-if="itemSearches[idx].open && searchResults.length > 0"

...
function openEdit(user: TenantUser) {
    ...
    editForm.password = "";
    editForm.role_id = user.role_id ? String(user.role_id) : "";
    editForm.is_active = user.is_active;
    editDialog.value = true;
}

function update() {
    editForm.put(route("tenant.users.update", editing.value!.id), {
        onSuccess: () => {
...
// ── Toggle activo ──────────────────────────────────────
function toggleActive(user: TenantUser) {
    router.patch(route("tenant.users.toggle-active", user.id));
}

// ── Eliminar ───────────────────────────────────────────
const deleteDialog = ref(false);
const toDelete = ref<TenantUser | null>(null);
...
function handleDelete() {
    router.delete(route("tenant.users.destroy", toDelete.value!.id), {
        onFinish: () => {
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead>Usuario central</TableHead>
        ...
        <Dialog v-model:open="createDialog">
            <DialogContent class="max-w-lg">
                <DialogHeader>
                    ...
                    <DialogDescription>Completa los datos para registrar un usuario.</DialogDescription>
                </DialogHeader>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2 flex items-center gap-3">
                            <Switch :model-value="createForm.is_active"
                                @update:model-value="(val) => (createForm.is_active = val)" />
        ...
        <Dialog v-model:open="editDialog">
            <DialogContent class="max-w-lg">
                <DialogHeader>
                    ...
                    <DialogDescription>Modifica los datos de <strong>{{ editing?.name }}</strong>.</DialogDescription>
                </DialogHeader>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2 space-y-2">
                            <Label>Nueva contraseña
                                <span class="text-muted-foreground text-xs">(dejar vacío para no cambiar)</span>
                            </Label>
                        ...
                        <div class="col-span-2 flex items-center gap-3">
                            <Switch :model-value="editForm.is_active"
                                @update:model-value="(val) => (editForm.is_active = val)" />
                            <Label class="cursor-pointer">Usuario activo</Label>
                        </div>
                    </div>
                    <DialogFooter>
        ...
        <Dialog v-model:open="deleteDialog">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Estás a punto de eliminar a <strong>{{ toDelete?.name }}</strong>.
